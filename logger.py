"""
Rich logging utilities adapted from VASA-1
...
log_level_str = os.environ.get('HRR_LOG_LEVEL', 'INFO').upper()
log_level_map = {
    'DEBUG': logging.DEBUG,
    'INFO': logging.INFO,
...
# Silence TensorFlow if installed
try:
...
# Suppress MediaPipe's verbose C++ logs
...
# Shared logger
logger = logging.getLogger("hrr_titans")
...
class TorchDebugger:
    ...
    def debug_model(
        ...
        logger.debug(f"\n[bold]Parameter Count:[/]")
        logger.debug(f"  Total parameters: {total_params:,}")
        logger.debug(f"  Trainable parameters: {trainable_params:,}")

        # Check each parameter
        logger.debug("\n[bold]Parameter Details:[/]")
        for name, param in model.named_parameters():
            logger.debug(
        ...
        if sample_input is not None or input_shape is not None:
            try:
                ...
                logger.debug(f"\n[green]Test forward pass successful![/]")
                logger.debug(f"Input shape: {test_input.shape}")
                logger.debug(f"Output shape: {output.shape}")
            except Exception as e:
                logger.error(f"[red]Error in test forward pass: {str(e)}[/]")
